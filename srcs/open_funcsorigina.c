...
/*   Updated: 2020/02/10 16:26:16 by cleisti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		printf("nb: %d\n", nb);
		get = (ptr->prec && ptr->prec_w < 1) ? ft_strdup("") : ft_itoa(nb);
		ptr->flag[2] = (ptr->flag[2] && nb > 0) ? 1 : 0;
	//	printf("prec: %d | prec_w: %d | get: %s | flag[2]: %d\n", ptr->prec, ptr->prec_w, get, ptr->flag[2]);
	}
	else
		get = convert(args, ptr);
	if (ptr->prec_w >= (int)ft_strlen(get))
		get = add_zeros(get, ptr->prec_w, ptr->neg);
//	printf("get1: %s | ptr->flag[3]: %d | ptr->w: %d\n", get, ptr->flag[3], ptr->w);
